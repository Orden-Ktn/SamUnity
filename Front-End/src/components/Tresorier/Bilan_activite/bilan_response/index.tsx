...
export default function Ajout_bilan() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (
      !formData.activite ||
      !formData.effectif ||
      !formData.montantdepense ||
      !formData.montantrecolte ||
      !formData.benefice ||
      !formData.perte
    ) {
    ...
    try {
      const response = await fetch(
        "http://localhost:8000/api/app/ajout_point_activite/",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <>
      <Breadcrumb pageName="Ajout Point d'activité" />
