...
export default function Ajout_depense() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8000/api/app/ajout_penalite/",
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        setErrors([data.message || "Erreur lors de l'ajout de la pénalité."]);
      }
    } catch (error) {
  ...
  return (
    <>
      <Breadcrumb pageName="Ajout pénalité" />
        ...
        <div className="flex w-full rounded-[10px] border-l-6 border-red-light bg-red-light-5 px-7 py-8 dark:bg-[#1B1B24] dark:bg-opacity-30 md:p-9">
          ...
          <div className="w-full">
            <h5 className="mb-2 font-bold leading-[22px] text-[#BC1C21]">
              Erreur lors de l'ajout de la pénalité.
            </h5>
            <ul>
