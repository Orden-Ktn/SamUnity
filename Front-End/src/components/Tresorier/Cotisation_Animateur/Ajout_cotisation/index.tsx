...
export default function Ajout_cotisation_animateur() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await fetch("http://localhost:8000/api/app/users/", {
          method: "GET",
          headers: {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!formData.mois || !formData.nom || !formData.montant) {
    ...
    try {
      const response = await fetch(
        "http://localhost:8000/api/app/ajout_cotisation_animateur/",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        setErrors(data.message || "Erreur lors de l'ajout de la cotisation.");
      }
    } catch (error) {
      setErrors("Erreur de connexion au serveur.");
    }
  ...
  return (
    <>
      <Breadcrumb pageName="Ajout Cotisation Animateur" />
        ...
        <div className="flex w-full rounded-[10px] border-l-6 border-green bg-green-light-7 px-7 py-8 dark:bg-[#1B1B24] dark:bg-opacity-30 md:p-9">
          ...
          <div className="w-full">
            <h5 className="mb-1 font-bold leading-[40px] text-[#004434] dark:text-[#34D399]">
              Cotisation mensulle enregistrée !
            </h5>
          </div>
        </div>
      )}
              ...
              <div className="flex flex-col gap-5.5 p-6.5">
                ...
                <div>
                  ...
                  <select
                    ...
                    value={formData.nom}
                    onChange={handleChange}
                    className="disabled:bg-gray-2 w-full rounded-[7px] border-[1.5px] border-stroke bg-transparent px-5.5 py-3 text-dark outline-none transition focus:border-primary active:border-primary disabled:cursor-default dark:border-dark-3 dark:bg-dark-2 dark:text-white dark:focus:border-primary"
                  >
                    <option value="">Sélectionnez celui qui fait le paiement</option>
                    {users.length > 0 ? (
                      users.map((user) => (
                  ...
                  {erreurs && <div className="text-red-500">{erreurs}</div>}
                </div>
