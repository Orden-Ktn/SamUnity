...
export default function Bilan_activite() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("http://localhost:8000/api/app/ajout_activite/", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        setErrors(data.intitule || "Erreur lors de l'ajout de l'activité.");
      }
    } catch (error) {
  ...
  return (
    <>
      <Breadcrumb pageName="Ajout Activité" />
      ...
      <div className="grid grid-cols-1 gap-9 sm:grid-cols-1">
        <div className="flex flex-col gap-9">
          ...
          {errors && (
            <div className="flex w-full rounded-[10px] border-l-6 border-red-light bg-red-light-5 px-7 py-8 dark:bg-[#1B1B24] dark:bg-opacity-30 md:p-9">
              ...
              <div className="w-full">
                <h5 className="mb-2 font-bold leading-[22px] text-[#BC1C21]">Erreur lors de l'ajout de l'activité.</h5>
                <ul>
        ...
        <div className="overflow-x-auto">
              ...
              {savedData.length === 0 && (
                <p className="mt-4 text-center">Aucune activité enregistrée.</p>
              )}
            </div>
