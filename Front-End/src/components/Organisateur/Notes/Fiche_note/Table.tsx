  ...
  return (
    <div className="grid grid-cols-1 gap-9 sm:grid-cols-1">
      <div className="flex flex-col gap-9">
        ...
        <div ref={tableRef} className="p-6.5 bg-white rounded-[10px] border">
          {loading ? (
              ...
              <table className="w-full border-collapse border border-[#ddd]">
                ...
                <tbody>
                  {results.map((result, index) => (
                    <tr key={result.id}>
                      <td className="border px-4 py-2 text-dark text-center">{result.nom}</td>
                      <td className="border px-4 py-2 text-dark text-center">{result.prenom}</td>
                      <td className="border px-4 py-2 text-dark text-center">
                      ...
                      <td className="border px-4 py-2 text-dark text-center">
                        Interros : <em className="font-bold"> {result.moyenne_interro ? result.moyenne_interro.toFixed(2) : "N/A"} </em> <br />
                        Générale: <em className="font-bold"> {result.moyenne_generale ? result.moyenne_generale.toFixed(2) : "N/A"} </em>
                        </td>
                    </tr>
                  ))}
                </tbody>
              </table><br />
              <p>NB: La moyenne des interrrogations est calculée avec les deux fortes notes d'interrogation.</p>
            </div>
          ) : (
