...
export default function Ajout_objectif() {
  ...
  const fetchObjectifs = async () => {
    try {
      const response = await fetch("http://localhost:8000/api/app/all_objectifs_organisation/");
      if (response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("http://localhost:8000/api/app/ajout_objectif_organisation/", {
        method: "POST",
        headers: {
